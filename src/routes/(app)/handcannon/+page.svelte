<script>
  ...
  let controllerInitialized = $state(false);
  ...
  // --- Populate Context Accessories and Set Initial Accessory ---
  $effect.pre(() => {
    if (browser) {
      // Populate accessories in context ONLY IF EMPTY
      if (context.currentGameAccessories.length === 0) {
        console.log('[Camera Page Effect] Populating context.gameAccessories');
        ...
        if (initialAccName) {
          ...
          if (initialIndex < 0) {
            console.warn(`[Camera Page] Initial accessory '${initialAccName}' not found in local list. Defaulting to first.`);
            initialIndex = 0;
          }
        } else {
          initialIndex = 0;
          console.log("[Camera Page] No initial accessory specified. Defaulting to first.");
        }
        ...
        if (context.selectedAccessoryIndex === -1) { // Check if not already set
            ...
            console.log(`[Camera Page] Set selectedAccessoryIndex to: ${initialIndex} (${HAND_ACCESSORIES_LIST[initialIndex]?.name})`);
        }
      }
    }
  });
  ...
  $effect.pre(() => {
    if (handController && !controllerInitialized) {
      console.log('[Camera Page Effect] Setting activeController in context (once).');
      ...
      return () => {
        if (context.activeController === handController) {
          console.log('[Camera Page Effect Cleanup] Clearing activeController from context.');
  ...
  // Dynamically determine which component to render based on the selected index
  const SelectedAccessoryComponent = $derived(
    HAND_ACCESSORIES_LIST[context.selectedAccessoryIndex]?.component
  );
...
<!-- Render the ground plane when scene is ready -->
{#if sceneReady && context.threeJsScene}
  <GroundPlane 
...
{#if shouldRenderAccessory}
  ...
  {#key context.selectedAccessoryIndex} 
    <Accessory
      scene={context.threeJsScene}
